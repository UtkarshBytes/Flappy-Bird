...
def run_flappy_bird():
    ...
    def reset_game():
        ...
        score = 0
        nonlocal collision_sound_played
        collision_sound_played = False
        return score
    ...
    class Bird(pygame.sprite.Sprite):
        ...
        def update(self):
            nonlocal flying, game_over, collision_sound_played, score, pass_pipe
            if flying == True:
                self.vel += 0.5
                if self.vel > 8:
                    self.vel = 8
    ...
    while run:
        ...
        if pygame.sprite.groupcollide(bird_group, pipe_group, False, False) or flappy.rect.top < 0 or flappy.rect.bottom >= 500:
            game_over = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if event.type == pygame.MOUSEBUTTONUP and flying == False and game_over == False:
                flying = True
...
while flag:
    ...
    light.draw(win)
    map.draw(win)
    run_flappy_bird()  # Run Flappy Bird game within the main loop
    pygame.display.update()
